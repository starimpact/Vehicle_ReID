...
def CreateModel_Color(ctx, batch_size, imagesize, clsnum):
  print 'creating network model2_softmax...'
  assert(imagesize[0]==299 and imagesize[1]==299)
  reid_net = create_reid_net(clsnum)

  return reid_net 


def draw_inception_renet_v2():
  reid_net = create_reid_net(clsnum=1024)
  #darw net
  datashape = (1, 3, 299, 299)
  graph = mx.visualization.plot_network(reid_net, shape={'data':datashape, 'label':datashape[0]})
  graph.render('inception_renet_v2_softmax') 
